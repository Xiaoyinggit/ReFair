...
class RNNdata(data.Dataset):
    ...
    def geneSamples(self):
        ...
        print('[RNNdata] gene %d %s samples; ns_pos_ratio: %d (%f), rating_pos_ratio: %f'%(len(self.user_feat_l), self.d_type, ns_stat,ns_stat/len(self.user_feat_l), label_stat/len(self.user_feat_l)))
    ...
    def loadData(self, file_path):
       ...
       item_feats_vec = []
       for i  in range(len(data_dict['item_feat_raw'])):
           ifl = data_dict['item_feat_raw'][i]
           # binary cat vector 
           cat_l = [0 for _ in range(data_dict['if_count'][1])]
           for catInd in ifl[1]:
              cat_l[catInd] = 1.0 / len(ifl[1])
           item_feats_vec.append(cat_l)
       data_dict['item_feats'] = item_feats_vec
...
def load_item_feats(data_path):
      ...
      for i  in range(len(item_feat_raw)):
           ifl = item_feat_raw[i]
           # binary cat vector 
           cat_l = [0 for _ in range(if_count[1])]
           for catInd in ifl[1]:
              cat_l[catInd] = 1.0 / len(ifl[1])
           item_feats_vec.append(cat_l)
