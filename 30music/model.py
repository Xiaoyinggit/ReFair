...
class pretrainRNNModel(torch.nn.Module):
    ...
    def __init__(self, model_config, item_feats, device='cuda',train_batch_size=512):
        ...
        # modules 
        self.userID_embs = torch.nn.Embedding(int(model_config['user_count']), self.dim, device=self.device) #[U, d]
        self.user_feat_embs = torch.nn.ModuleList([])
        for i in range(1, len(uf_count)):
            self.user_feat_embs.append(torch.nn.Embedding(uf_count[i], self.dim, device=self.device))

        
        self.if_count = json.loads(model_config['if_count']) #[itemid, artist, album, cat]
        self.itemID_embs = torch.nn.Embedding(int(model_config['item_count']), self.dim, device=self.device) #[I,d]
        self.item_cat_embs = torch.nn.Embedding(int(self.if_count[-1]), self.dim, device=self.device, dtype=torch.float64) #[C, d]
        self.item_artist_embs = torch.nn.Embedding(int(self.if_count[1]), self.dim, device=self.device, dtype=torch.float64) 
        self.item_album_embs = torch.nn.Embedding(int(self.if_count[2]), self.dim, device=self.device, dtype=torch.float64) 

        self.item_feats = torch.tensor(item_feats, device=self.device, requires_grad=False, dtype=torch.float64) # [I, 2+cat_num]
        ...
        self.item_linear_proj = torch.nn.Linear(len(self.if_count)*self.dim, self.dim, bias=False, device=self.device, dtype=torch.float64) #[itemID + cat --> dim]
    ...
    def cal_loss(self, pred, r_label, ns_label, summarywriter=None):
        ...
        loss_info= {'rating_mse':rating_pred_loss, 'ns_mse': self.ns_loss_weight* ns_pred_loss}
        ...
        loss_sum = rating_pred_loss + self.ns_loss_weight* ns_pred_loss
    ...
    def item_tower(self, itemIDs):
        ...
        b_item_artist_ind, b_item_album_ind, b_item_cats_ind = torch.split(b_item_feats,[1,1,self.if_count[-1]],dim=-1)
        b_item_artist_ind = b_item_artist_ind.squeeze(dim=-1)
        b_item_album_ind = b_item_album_ind.squeeze(dim=-1)


        b_item_artist_emb = self.item_artist_embs(b_item_artist_ind.to(torch.int64))

        b_item_album_emb = self.item_album_embs(b_item_album_ind.to(torch.int64))
 

        b_item_cat_emb = torch.matmul(b_item_cats_ind, self.item_cat_embs.weight) #[B, C]*[C,d]--> [B,d]

        b_item_featEmb = torch.concat([b_item_id_emb,b_item_artist_emb, b_item_album_emb, b_item_cat_emb], dim=-1) 
    ...
    def user_tower(self, user_feats, hist_seq, seq_len):
        ...
        user_feats = user_feats.to(dtype=torch.int64)
